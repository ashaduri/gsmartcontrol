...
namespace {
	...
	struct AttributeDescription {
		...
		/// Constructor
		AttributeDescription(int32_t id_, std::optional<StorageDeviceDetectedType> type, std::string reported_name_,
...
// 						"");
			...
			{
				add(AttributeDescription(id, std::nullopt,
...
// 			}
			...
			[[nodiscard]] AttributeDescription find(const std::string& reported_name, int32_t id, std::optional<StorageDeviceDetectedType> type) const
			{
				// search by ID first
...
// 				|| auto_set(p, "error_log_unsupported", "This device does not support error logging.");  // the property text already says that
				if (p.is_value_type<AtaStorageErrorBlock>()) {
					for (std::size_t i = 0; i < p.get_value<AtaStorageErrorBlock>().reported_types.size(); ++i) {
		...
		StoragePropertyRepository properties, StorageDeviceDetectedType device_type)
{
	for (auto& p : properties.get_properties_ref()) {
		ata_storage_property_autoset_description(p, device_type);
