...
// 		}
		...
		/// Get last operation's errno if it was unsuccessful.
		int get_errno() const
		{
			return error_errno_;
		}
	...
	protected:
		...
		/// Set error (formatted message, errno, up to two affected paths).
		/// The formatted message may contain the following special strings:
		/// - /path1/ - replaced by \c path1;
		/// - /path2/ - replaced by \c path2;
		/// - /errno/ - replaced by errno string.
		...
		/// Import error from another object
		...
		/// This is called on every set_error(). Override to e.g. enable GUI notifications.
		/// The default implementation prints a warning using libdebug.
		virtual void warn()
		{
